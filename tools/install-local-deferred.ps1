...
$jars = @(Get-ChildItem -LiteralPath $PSScriptRoot -Filter 'TropimonBetterPC-*-LOCAL.jar' -File)
...
& (Join-Path $PSScriptRoot 'InstallManagedLocalMod.ps1') -SourceJar $jars[0].FullName `
    -ExpectedModId 'tropimon_better_pc' -LauncherRoot $LauncherRoot -PollSeconds $PollSeconds
exit $LASTEXITCODE
