...
$artifact = Join-Path $project "build/libs/tropimon-better-pc-$modVersion.jar"
if (!(Test-Path -LiteralPath $artifact)) { throw 'Build the release JAR first.' }
Get-ChildItem -LiteralPath $mods -Filter 'tropimon-better-pc-*.jar' -File |
...
$arguments = @('-Xmx3G', '-Dbetterpc.smoke=true', "-Dbetterpc.smoke.language=$Language", '-Dfabric.log.disableAnsi=true',
...
if (-not (Select-String -LiteralPath (Join-Path $run "logs/latest.log") -SimpleMatch "BETTER_PC_SMOKE_OK" -Quiet)) { throw "Verification incompletement validee : consulter le journal local." }
