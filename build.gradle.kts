...
val privateContent = listOf(
    ...
    "**/saves/**", "**/sessions/**", "**/*.p12", "**/*.pfx", "**/*.keystore", "**/*.pem"
)
...
val testLocalInstaller by tasks.registering(Exec::class) {
    onlyIf { System.getProperty("os.name").startsWith("Windows") }
    commandLine("powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File",
        file("tools/TestInstaller.ps1").absolutePath)
}
tasks.check { dependsOn(privacyCheck, testLocalInstaller) }
...
// This offline test helper is never packaged in either delivered JAR.
...
loom {
    mods {
        create("tropimon_better_pc") { sourceSet(sourceSets.main.get()) }
        create("better_pc_smoke") { sourceSet(smoke) }
    }
    runs {
        create("smoke") {
            client()
            ...
            vmArg("-Dbetterpc.smoke=true")
            vmArg("-Dfabric.debug.disableErrorGui=true")
            vmArg("-Xmx3G")
            programArgs("--username", "BetterPcTest", "--width", "1400", "--height", "900")
        }
    }
}

tasks.register<Exec>("armBetterPcLocal") {
    ...
    commandLine("powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File",
        ...
        "-Source", localDir.map { it.file("TropimonBetterPC-${project.version}+1.21.1-LOCAL.jar").asFile.absolutePath }.get())
}
...
val prepareReleaseDelivery = tasks.register("prepareReleaseDelivery") {
    ...
    doLast {
        ...
        copyAndHash(shareDirectory.resolve("TropimonBetterPC-${project.version}+1.21.1.jar"))
